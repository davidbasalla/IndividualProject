...
state issues with XTK not being designed to load things dynamically. Desired for this viewer.
issuse with NRRD files, bug

\subsubsection{Colortables}

Colortable loading via XTK\\
Having to understand the loading process in depth\\
Hardcoding arrays into the code\\
Losing flexibility, but faster and more efficient


\subsubsection{Annotations}

XML format - first\\
Json Format - more convenient, logical extension and more natural given javascript nature\\
new Annotation class\\
functions for converting 2D to 3D and 3D to 2D (inverting matrix mulitplication)\\
taking colopicker from online\\
...
I downloaded a couple of ColorPickers from the web, and deemed Blah's colorpicker to be better suited. It has a very basic interface that was deemed sufficient for chosing an Annotation color. Installing and implementing it into the project was also very straight forward.
